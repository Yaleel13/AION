...
def build_runtime_status() -> dict[str, Any]:
    ...
    Demo / fixture UI surfaces must not invent these fields — they come from
    env + durable storage + policy loaders only.
    """
    ...
    try:
        ...
        moltbook = {
            "configured": moltbook_settings.is_mock
            or moltbook_settings.configured_for_live,
    ...
    autonomy_mode = policy.mode.value
    live_writes = bool(
    ...
    return {
        ...
        "paper_market_data": {
            ...
            "live_trading": False,
            "note": (
                "Paper trading only. Official metrics separate live public "
                "market marks from mock/fallback prices."
            ),
        },
        "providers": {
