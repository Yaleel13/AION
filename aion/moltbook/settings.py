...
class MoltbookSettings:
    """Validated settings for AION's Moltbook integration.

    Live reads remain available independently. Controlled outbound is a separate,
    explicit two-flag capability and is never enabled by default.
    """
    ...
    user_agent: str = "AION-Moltbook-Emissary/0.3-controlled-owner-outbound"
    ...
    def controlled_outbound_ready(self) -> bool:
        return bool(
            self.configured_for_live
            and self.outbound_enabled
            and self.execute_enabled
        )
...
def load_moltbook_settings(
    *,
    ...
    execute_enabled = _parse_bool(env.get("MOLTBOOK_PHASE2_EXECUTE"), False)
    ...
    if (outbound_enabled or execute_enabled) and mode != "live":
        raise MoltbookConfigError(
            "Controlled outbound flags require MOLTBOOK_MODE=live"
        )
    if execute_enabled and not outbound_enabled:
        raise MoltbookConfigError(
            "MOLTBOOK_PHASE2_EXECUTE=true requires MOLTBOOK_OUTBOUND_ENABLED=true"
        )
