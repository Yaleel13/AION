          ...
          <>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={running}
                className="inline-flex items-center gap-2 rounded-md bg-foreground px-3 py-2 text-xs font-medium text-background disabled:cursor-not-allowed disabled:opacity-50"
              >
            ...
            {result ? (
              <pre className="whitespace-pre-wrap break-words rounded-lg border border-border bg-black/20 p-4 font-mono text-[0.72rem] leading-relaxed text-foreground">
...
arbitrary commands enabled: ${String(result.arbitraryCommandsEnabled ?? false)}`}
              </pre>
            ) : null}

            <p className="text-[0.7rem] leading-relaxed text-muted-foreground">
              This first executor release intentionally does not expose an arbitrary shell. Repository writes, deployments, destructive commands, credential operations, and other consequential actions remain unavailable until separately reviewed and approval-gated.
            </p>
          </>
        ) : (
          <div className="mx-auto my-auto w-full max-w-md rounded-xl border border-caution/30 bg-caution/5 p-5">
            ...
            <div className="mt-4 flex gap-2">
              <input
                ...
                autoComplete="off"
                spellCheck={false}
                placeholder="AION owner token"
                className="min-w-0 flex-1 rounded-md border border-border bg-background/60 px-3 py-2 text-sm text-foreground outline-none focus:border-gold/50"
              />
              <button
                type="button"
                onClick={unlock}
                disabled={!token.trim() || loading || !session.configured}
            ...
            {!session.configured ? (
              <p className="mt-3 text-xs text-caution">AION_OWNER_TOKEN is not configured on this deployment.</p>
            ) : null}
          </div>
        )}
