...
async function proxy(req: Request, method: "GET" | "POST") {
  ...
  try {
    ...
    const response = await fetch(new URL("/api/internal/commercial-execution", req.url), {
      method,
      headers: {
    ...
    console.error("[AION] Commercial execution proxy error:", error instanceof Error ? error.message : String(error))
    return Response.json({ error: "Commercial execution controls are temporarily unavailable." }, { status: 500 })
  }
}
